...
const handlers = {
	...
	onSaveBtnClick : (e) => {
		e.preventDefault();
		const target = $(e.target);
		const type = target.closest('form').attr('id');
		const input = target.closest('form').find('input')[0];
...
$('#alarm-set').click(handlers.onAlarmSetterClick);
$('.btn-set').click(handlers.onSaveBtnClick);
